function SXPAR_CHARIS, hdr, name, abort, count = matches, comment = comments, justfirst = justfirst, $
  nocontinue = NoContinue, silent = silent
  ; ****************
  ; **a patch to sxpar.pro to force it to return the first instance of a keyword, only
  ; ****************
  ...
  if vector then begin
    ...
    if keyword_set(justfirst) then begin
      nfound = nfound[0]
      matches = 1
    endif
