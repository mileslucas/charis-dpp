pro charis_aloci_fwdmod_planet, pfname, reducname = reducname, sdi_reducname = sdi_reducname, $
  nonorthup = nonorthup, $
  prefname = prefname, gauspsf = gauspsf, method = method, $
  planetmethod = planetmethod, planetmodel = planetmodel, targfile = targfile, $
  pickpsf = pickpsf, $
  reductype = reductype, $
  adi = adi, sdi = sdi, asdi = asdi, rdi = rdi, refpath = refpath, help = help
  compile_opt idl2

  ; ****Version 2.2 - added SDI+p-ADI
  ; ****Version 2.1 - put in decision tree to handle ADI, SDI, ADI+p-SDI, [and in future - ASDI]
  ; ****Version 2.0 - cleaned up code
  ; *****06/03/2020
  ; ;
  ; ****12/27/2018**
  ; can do multiple planets
  ; ***
  ; ***04/21/2018**
  ...
  if (n_params() eq 0 or keyword_set(help)) then begin
    print, 'charis_aloci_fwdmod_planet: A-LOCI fwd-modeling of a point source, Version 2.0 (June 2020)'
    print, 'Written by T. Currie (2014), adapted for CHARIS IFS Data (2019)'
    print, ''
    print, 'charis_aloci_fwdmod_planet,pfname,reducname=reducname,reductype=reductype,sdi_reducname=sdi_reducname,'
    print, 'method=method,planetmethod=planetmethod,planetmodel=planetmodel,targfile=targfile,adi=adi,sdi=sdi,asdi=asdi,'
    print, 'rdi=rdi,refpath=refpath'
    print, 'dlrstep=dlrstep,lrmin=lrmin,lrmax=lrmax'
    ; print,"rdi=rdi,asdi=asdi,"
    print, 'pickpsf=pickpsf,gauspsf=gauspsf,contrast=contrast'
    print, 'reducname=reducname,prefname=prefname'
    print, ''
    print, 'Example (ADI):'
    print, 'charis_aloci_fwdmod_planet,''HR8799_low.info'',reducname=''aloci.fits'',method=0'
    print, ''
    print, 'Example (SDI):'
    print, 'charis_aloci_fwdmod_planet,''HR8799_low.info'',reducname=''aloci.fits'',method=0,reductype=1'
    print, ''
    print, 'Example (ADI, p-SDI):'
    print, 'charis_aloci_fwdmod_planet,''HR8799_low.info'',reducname=''aloci.fits'',sdi_reducname=''sdi.fits'',method=0,reductype=2'
    print, ''
    print, 'Example (SDI, p-ADI):'
    print, 'charis_aloci_fwdmod_planet,''HR8799_low.info'',reducname=''aloci.fits'',sdi_reducname=''sdi.fits'',method=0,reductype=3'
    print, ''
    ...
    print, '*method - 0 [manually enter x,y, and log(contrast) at prompt], 1 [select a file w x,y,contrast], 2[use GUI to select a file containing x,y,contrast from GUI]'
    ...
    print, '*targfile - name of file with x,y,contrast [if method eq 1 or ''targfile'']'
  ...
  ; if keyword_set(asdi) then reductype = 4 ;set to ASDI
  ...
  if ~keyword_set(method) then begin
    ...
    case method of
      ...
      'targfile': begin
        readcol, targfile, xp0, yp0, contrast
  ...
  ; help,planet_input_spectrum
  ; help,median(planet_input_spectrum,dimension=3,/even)

  ; average input planet spectrum
  planet_avg_input_spectrum = median(planet_input_spectrum, dimension = 3, /even)
  ; print,planet_avg_input_spectrum[*,0]
  ...
  case reductype of
    0: begin ; just ADI
      ...
      ; /usecoeff,/fc,/fwdmod,outfile='res'+string(itc,format='(i1)')+'.fits'
    end
    ...
    1: begin ; just SDI
      ...
      ; /usecoeff,/fc,outfile='res'+string(itc,format='(i1)')+'.fits'
    end
    ...
    3: begin ; SDI, ADI on post-SDI residuals
      ...
      charis_sdialoci, /fwdmod, pfname, prefname = prefname, /norot, $
        ; charis_sdialocif,/fwdmod,pfname,prefname=prefname,zonetype=zsdiztype,$
        nfwhm = sdiznfwhm, drsub = sdizdrsub, na = sdizna, geom = sdizgeom, rmin = sdizrmin, rmax = sdizrmax, svd = sdizsvd, nref = sdiznref, pixmask = sdizpixmask, zero = sdizzero, meanadd = sdizmeanadd, $
        ; /usecoeff,
        /fc, /norot
      ...
      charis_adialoci, pfname, prefname = prefname, rsubval = zrsub, $
        ; charis_adialocif,pfname,prefname=prefname,rsubval=zrsub,zonetype=zadiztype,$
        nfwhm = znfwhm, drsub = zdrsub, na = zna, geom = zgeom, rmin = zrmin, rmax = zrmax, svd = zsvd, nref = znref, pixmask = zpixmask, zero = zzero, meanadd = zmeanadd, $
        suffname = '_sdialocisub', /fc, /fwdmod, /postsdi, outfile = 'res' + string(itc, format = '(i1)') + '.fits'
