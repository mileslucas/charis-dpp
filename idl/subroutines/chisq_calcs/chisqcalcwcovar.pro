pro chisqcalcwcovar, obs = obs, eobs = eobs, model_in = model_in, err_model_in = err_model_in, alpha_in = alpha_in, speccovar = speccovar, $
  chisqout = chisqout, ndof = ndof, alpha_out = alpha_out, good = good, phot = phot, ephot = ephot, modelphot = modelphot
  compile_opt idl2
  ...
  for i = 0l, nsfac - 1 do begin
    ...
    if gval lt chisqtot then begin
      ...
      ; print,'1',transpose(diff)#invert(cmaterr,/double)#diff
      ; print,'2',total((diffphot/ephot)^2.)
      ; print,'3',chisqtot
    endif
  endfor
